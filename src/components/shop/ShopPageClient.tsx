  ...
  return (
    <div className='min-h-screen bg-[var(--dt-bg-page)] dt-font-body'>
      <Hero
        ...
        title={<>Take the <em className='not-italic text-[var(--dt-shop-purple)]'>Magic Home</em></>}
      ...
      <div className='max-w-[1200px] mx-auto px-6 py-8 pb-20'>
        <FlowSteps
          ...
          steps={[
            { title: 'Pick an item', description: 'Choose toys, apparel, furniture, or branded gear from any section.' },
            { title: 'Set fulfillment', description: 'Choose pickup or delivery, add quantity, and set your preferred handoff date.' },
            { title: 'Send the request', description: 'Review the order, enter contact details, and complete the frontend purchase flow.' },
          ]}
        />
        ...
        {/* Sections */}
        {SHOP_SECTIONS.map((section, index) => {
          ...
          return (
            <React.Fragment key={section.id}>
                ...
                {items.length === 0 ? (
                  <div className='py-12 text-center text-[var(--dt-text-subtle)]'>
                    ...
                    <div className='text-[15px] font-bold'>
                      {section.emptyText.replace('{filter}', activeFilter)}
                    </div>
                  </div>
                ) : (
              ...
              {index < SHOP_SECTIONS.length - 1 ? <div className='dt-section-divider' /> : null}
            </React.Fragment>
          )
        })}
