...
import type { WhyItemCardProps } from './types'

import { useState } from 'react'
import type { WhyItem } from './types'

export default function WhyItemCard({ item }: WhyItemCardProps) {
  ...
  return (
    <div
      ...
      className={[
        ...
        hover ? '-translate-y-1 shadow-[0_20px_48px_rgba(12,166,120,0.13)]' : 'shadow-[0_2px_10px_rgba(0,0,0,0.05)]',
      ].join(' ')}
    >
      {/* Image */}
      <div className='h-[180px] relative overflow-hidden'>
        ...
        {/* Dot indicator */}
        <div className='absolute top-3.5 left-3.5 w-2.5 h-2.5 rounded-full bg-[#0CA678] shadow-[0_0_0_3px_rgba(12,166,120,0.27)]' />
      </div>
      ...
      <div className='px-5 py-5 flex flex-col h-[calc(100%-180px)]'>
        ...
        <p className='text-[13px] leading-[1.6] text-[var(--dt-text-muted)] mt-auto'>{item.desc}</p>
      </div>
    </div>
  )
}
