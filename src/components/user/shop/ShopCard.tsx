...
export default function ShopCard({
  ...
  return (
    <div className='dt-card-interactive flex flex-col' style={{ borderColor: item.border }}>
      <div className='h-[5px]' style={{ background: item.color }} />

      {item.badge ? (
        <div className='absolute ml-auto mr-3 mt-3 w-fit self-end' style={{ background: badgeColor }}>
      ...
      <div className='flex-1 px-[22px] pb-4 pt-5'>
        <img
          src={item.image}
          alt={item.name}
          className='w-[64px] h-[64px] object-cover rounded-[14px] mb-3 shadow-sm'
        />
        ...
        <div className='mb-3 flex flex-wrap gap-1.5'>
          ...
          {showMaterial && item.material ? <span className='dt-pill-neutral'>{item.material}</span> : null}
        </div>
      </div>
